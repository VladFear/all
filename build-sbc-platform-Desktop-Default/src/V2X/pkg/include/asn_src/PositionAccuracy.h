/*
 * Generated by asn1c-0.9.29 (http://lionet.info/asn1c)
 * From ASN.1 module "PositionFrameModule"
 * 	found in "./PositionFrameModule.asn"
 * 	`asn1c -gen-PER`
 */

#ifndef	_PositionAccuracy_H_
#define	_PositionAccuracy_H_


#include <asn_application.h>

/* Including external dependencies */
#include "SemiMajorAxisAccuracy.h"
#include "SemiMinorAxisAccuracy.h"
#include "SemiMajorAxisOrientation.h"
#include <constr_SEQUENCE.h>

#ifdef __cplusplus
extern "C" {
#endif

/* PositionAccuracy */
typedef struct PositionAccuracy {
	SemiMajorAxisAccuracy_t	 semiMajor;
	SemiMinorAxisAccuracy_t	 semiMinor;
	SemiMajorAxisOrientation_t	 orientation;
	
	/* Context for parsing across buffer boundaries */
	asn_struct_ctx_t _asn_ctx;
} PositionAccuracy_t;

/* Implementation */
extern asn_TYPE_descriptor_t asn_DEF_PositionAccuracy;
extern asn_SEQUENCE_specifics_t asn_SPC_PositionAccuracy_specs_1;
extern asn_TYPE_member_t asn_MBR_PositionAccuracy_1[3];

#ifdef __cplusplus
}
#endif

#endif	/* _PositionAccuracy_H_ */
#include <asn_internal.h>
